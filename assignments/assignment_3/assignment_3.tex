...
\begin{titlepage}
	...
	In accordance with UiA's guidelines, I declare that I used Generative AI tools to assist in structuring the LaTeX report (tables, and some formulas) and generating boilerplate code for plotting. The neural network implementation, error estimation analysis, model selection methodology, and interpretation of results are my own work, guided by the course lecture materials (Lectures 6--8) and exercises (7--9) + class attendance.
\end{titlepage}
...
\textbf{Test performance:} The model achieves \textbf{97.75\%} accuracy on the held-out test set (error rate: 2.25\%). Figure~\ref{fig:confusion_matrix} shows the confusion matrix. The most error-prone classes are digit 1 (3 errors: confused with 3 and 7) and digit 5 (3 errors: confused with 3, 4, and 9), which share stroke-level similarities in handwritten form.
...
$$\Xi_n(\Psi_n, S_n) = \frac{1}{n} \sum_{i=1}^{n} |Y_i - \Psi_n(S_n)(\mathbf{X}_i)|$$
...
The hold-out error (0.62\%) is noticeably lower than the true test error (1.75\%). One likely reason is that the hold-out set comes from the same pool of 2000 samples as the training data --- so the split is random within one collection, whereas the external test set was provided separately and may capture slightly different variation. On top of that, we only train on 1280 samples here instead of the full 1600, but since the hold-out portion ``looks like'' the training portion, the model ends up appearing better on it than it really is on unseen data.
...
\begin{table}[H]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		Resubstitution   & 0.00\%              & Severely Optimistic  \\
...
Plugging in $N = 3$ classifiers, $m = 240$ validation samples, and $\tau = 0.01$ (a 1\% error difference), we get $P \leq 2 \cdot 3 \cdot e^{-240 \cdot 0.0001/2} = 6 \cdot e^{-0.012} \approx 5.93$. Since this exceeds 1, the bound tells us nothing useful --- 240 samples simply are not enough to reliably tell apart models whose true errors differ by less than 1\%. This lines up with what we observed: the TVT approach could barely distinguish the three configurations.
...
This result also illustrates the \textbf{complexity--sample size tradeoff} (Lecture 8, slides 7--8). The error of a designed classifier decomposes as $\varepsilon_{n,\mathcal{C}} = \varepsilon^* + \Delta_{\mathcal{C}} + \Delta_{n,\mathcal{C}}$, where $\Delta_{\mathcal{C}}$ is the approximation error (smaller for more complex $\mathcal{C}$) and $\Delta_{n,\mathcal{C}}$ is the design error (larger for more complex $\mathcal{C}$ at fixed $n$). With $n = 1600$ samples, Config C's bigger model (90K parameters) brings $\Delta_{\mathcal{C}}$ down without blowing up $\Delta_{n,\mathcal{C}}$, since the strong dropout (0.4) keeps the effective complexity in check. With a smaller dataset, the simpler Config A would probably win out because the design error would start to dominate.

\section{Conclusion}
A fully connected neural network with ReLU activations, batch normalization, and dropout achieves 97.75\% accuracy on the mfeat-factors 10-class digit classification task. The error estimation experiments confirm that resubstitution is severely optimistically biased (0.00\% vs.\ true 1.75\%), hold-out partially corrects this (0.62\%) but remains optimistic due to single-split limitations, and 5-fold CV provides the most reliable estimate (2.50\%) with slight pessimistic bias from the reduced training set size, as predicted by $E[\hat{\varepsilon}_n] = E[\varepsilon_{n-n/k}]$ (Lecture 7). The hold-out RMS bound of $\leq 2.80\%$ with $m=320$ (Lecture 7, slide 20) shows how much the estimate can drift with a limited validation set. For model selection, CV was clearly more useful than TVT --- the selection probability bound (Lecture 8, slide 14) with only 240 validation samples comes out above 1, meaning it gives us no guarantee at all, while CV gets around this by using all 1600 samples across folds. The final model (Config C) hits a good spot in the complexity--sample size tradeoff $\varepsilon_{n,\mathcal{C}} = \varepsilon^* + \Delta_{\mathcal{C}} + \Delta_{n,\mathcal{C}}$ (Lecture 8, slides 7--8): more parameters to reduce approximation error, but enough dropout to keep overfitting under control.
