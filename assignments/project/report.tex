...
\begin{titlepage}
	...
	\vspace{1.5cm}
	\Large \textbf{Student:} Gormery K. Wanjiru \\
...
The training data is split 80/20 using stratified sampling, meaning the class proportions are preserved in both the training and validation subsets (e.g., each subset contains exactly 10\% of each clothing class). This prevents the validation set from accidentally over- or under-representing certain classes, which would bias the model selection. Each model is trained on the 80\% split with early stopping based on validation loss. The model with the highest validation accuracy is selected, then retrained on 90\% of the full training set (with 10\% held out for early stopping only).
...
\begin{itemize}[nosep]
	...
	\item Batch size of 256 is large enough to keep the GPU busy (avoiding idle cycles between batches) while still providing enough gradient noise to help escape local minima during optimization.
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{Random samples from each class in the Fashion MNIST training set. The visual similarity between T-shirt/Pullover/Coat/Shirt classes is clear.}
	\label{fig:samples}
\end{figure}
...
\begin{itemize}[nosep]
	...
	\item Several classes are visually similar (T-shirt, Pullover, Coat, Shirt all have similar silhouettes), which later on proved to be harder to distinguish with pixel-level features.
